...
root.title("GeNamenator v 2.31")
root.iconbitmap('Groucho_Icon.ico')

conn = sqlite3.connect('GeName_DB_3.db')
curs = conn.cursor()
...
def about_window():
    about = Tk()
    about.geometry("617x624")
    about.title("About GeNamenator v 2.3")
    about.iconbitmap('Groucho_Icon.ico')

    about_text = "GeNamenator began as a project in July 2020 to learn Python and SQL.  The first GUI-based \n" \
                 "version, v2.1, was completed in August 2020.\n" \
                 "\n" \
                 ...
                 "\n" \
                 "Last name commonness is based on the top 1,000 surnames taken from the 2010 census data.  \n" \
                 "The frequency of these names is based on United States census data, and names marked as \n" \
                 "rare are obviously much more common in other countries.\n" \
                 "Commonness is again based on an arbitrary rating system:\n" \
                 ...
                 "\n" \
                 "Due to how numbers work, random generation at any level is going to favor uncommon names \n" \
                 "unless specifically left out of searches.\n" \
                 "\n" \
                 "Research for this was done primarily through the US Social Security Administration and US Census \n" \
                 "websites, Wikipedia, www.babynamewizard.com for additional name popularity information, and \n" \
                 "adoption.com for additional name origin information.\n" \
                 "\n" \
                 "The database is not a scientifically gathered list of names and information may and will be incorrect.  \n" \
                 "That's why there's a DBM included.\n" \
                 "\n" \
                 "\t-David Margis, August 2020"

    about_frame = LabelFrame(about, font=("Times", 12), borderwidth=5, fg=text_color, background=frame_bg, relief=RIDGE)
    about_frame.grid(row=0, column=0)
    about_label = Label(about_frame, font=("Times", 12), text=about_text, justify=LEFT)
...
curs.execute(""" SELECT origin1, origin2, origin3, origin4, origin5, origin6, origin7 FROM last_names """)
...
def create_last_name():
    ...
    if allitt_choice == "AllitOff":
        ...
        if ln_origin_choice == "":
            ...
            curs.execute(""" SELECT last_name FROM last_names WHERE (?) IN (origin1, origin2, origin3, origin4, origin5, origin6, origin7) """,
                         (ln_origin_choice,))
            ln_pull = curs.fetchall()
            if lnhyph_choice == "HyphOn":
                last_name_first = random.choice(ln_pull)
        ...
        try:
            ...
            if ln_origin_choice == "":
                ...
                curs.execute(""" SELECT last_name FROM last_names WHERE last_name LIKE (?) AND (?) IN (origin1, origin2, origin3, origin4, origin5, origin6, origin7) """,
                             (allit_end_string, ln_origin_choice))
                ln_pull = curs.fetchall()
                if lnhyph_choice == "HyphOn":
                    last_name_first = random.choice(ln_pull)
                    if len(ln_pull) > 1:
...
f_m_label = Label(f_m_frame, text="Gender", font=(main_font, frame_label_font, 'bold'), fg=text_color, bg=frame_bg)
f_m_label.grid(row=0, column=0, columnspan=2)
f_radio = Radiobutton(f_m_frame, font=(main_font, option_label_font), text="Female", padx=5, variable=gender, value="Female", fg=text_color, bg=frame_bg)
f_radio.grid(row=1, column=0, sticky=W)
m_radio = Radiobutton(f_m_frame, font=(main_font, option_label_font), text="Male", padx=5, variable=gender, value="Male", fg=text_color, bg=frame_bg)
m_radio.grid(row=2, column=0, sticky=W)
a_radio = Radiobutton(f_m_frame, font=(main_font, option_label_font), text="Any", padx=5, variable=gender, value="Any", fg=text_color, bg=frame_bg)
...
middle_label = Label(middle_frame, font=(main_font, frame_label_font, 'bold'), text="Middle Name", fg=text_color, bg=frame_bg)
...
middle_check_label = Label(middle_frame, font=(main_font, option_label_font), text="Middle Name", fg=text_color, bg=frame_bg)
...
midlast_check_label = Label(middle_frame, font=(main_font, option_label_font), text="Last Name for\nMiddle Name", fg=text_color, bg=frame_bg)
...
ln_label = Label(ln_frame, font=(main_font, frame_label_font, 'bold'), text="Last Name", fg=text_color, bg=frame_bg)
...
lnhyph_check_label = Label(ln_frame, font=(main_font, option_label_font), text="Hyphenated", fg=text_color, bg=frame_bg)
lnhyph_check_label.grid(row=1, column=1, sticky=W)
ln_origin_combo = ttk.Combobox(ln_frame, font=(main_font, option_label_font), width=12, value=ln_origin_list, state="readonly")
ln_origin_combo.current(0)
ln_origin_combo.grid(row=2, column=1, padx=5, pady=5)
ln_origin_label = Label(ln_frame, font=(main_font, option_label_font), text="Origin", fg=text_color, bg=frame_bg)
...
allit_label = Label(allit_frame, font=(main_font, frame_label_font, 'bold'), text="Alliteration", fg=text_color, bg=frame_bg)
...
allit_check_label = Label(allit_frame, font=(main_font, option_label_font), text="Alliterative Name", fg=text_color, bg=frame_bg)
allit_check_label.grid(row=1, column=1, sticky=W)
allit_begin = Entry(allit_frame, font=(main_font, option_label_font), width=3, borderwidth=5)
allit_begin.grid(row=2, column=0, padx=5, pady=5)
allit_label = Label(allit_frame, font=(main_font, option_label_font), text="Name starts with", fg=text_color, bg=frame_bg)
...
decade_label = Label(decade_frame, font=(main_font, frame_label_font, 'bold'), text="Birth Decade", fg=text_color, bg=frame_bg)
decade_label.grid(row=0, column=0, columnspan=2)
decade_menu = ttk.Combobox(decade_frame, font=(main_font, option_label_font), width=10, value=dec_list, state="readonly")
decade_menu.current(0)
decade_menu.bind("<<ComboboxSelected>>", default_decom)
decade_menu.grid(row=1, column=1)
decade_menu_label = Label(decade_frame, font=(main_font, option_label_font), text="Decade", fg=text_color, bg=frame_bg)
decade_menu_label.grid(row=1, column=0)
commonness_label = Label(decade_frame, font=(main_font, frame_label_font, 'bold'), text="Commonness", fg=text_color, bg=frame_bg)
...
topten_label = Label(decade_frame, font=(main_font, option_label_font), text="Top Ten", fg=text_color, bg=frame_bg)
...
mostcommon_label = Label(decade_frame, font=(main_font, option_label_font), text="Most Common", fg=text_color, bg=frame_bg)
...
common_label = Label(decade_frame, font=(main_font, option_label_font), text="Common", fg=text_color, bg=frame_bg)
...
lesscommon_label = Label(decade_frame, font=(main_font, option_label_font), text="Less Common", fg=text_color, bg=frame_bg)
...
rare_label = Label(decade_frame, font=(main_font, option_label_font), text="Rare", fg=text_color, bg=frame_bg)
...
exrare_label = Label(decade_frame, font=(main_font, option_label_font), text="Extremely Rare", fg=text_color, bg=frame_bg)
...
clear_all = Button(root, font=(main_font, 18, 'bold'), text="Clear", padx=25, pady=10, fg=text_color, bg=button_bg, activebackground=button_act_bg, relief=RAISED, borderwidth=18, command=clear)
clear_all.grid(row=9, column=6, rowspan=1, columnspan=2, sticky=N+S+E+W)

close = Button(root, font=(main_font, 18, 'bold'), text="Close", padx=25, pady=10, fg=text_color, bg=button_bg, activebackground=button_act_bg, relief=RAISED, borderwidth=18, command=root.quit)
close.grid(row=14, column=6, rowspan=1, columnspan=2, sticky=N+S+E+W)
...
def database():
    ...
    # First Name
    fn_frame = LabelFrame(dbwin, font=(main_font, frame_label_font), borderwidth=5, fg=text_color, background=frame_bg, relief=RIDGE)
    fn_frame.grid(row=0, column=0, rowspan=5, columnspan=2, sticky=N)
    fn_label = Label(fn_frame, text="First Names", font=(main_font, frame_label_font, 'bold'), fg=text_color, bg=frame_bg)
    fn_label.grid(row=0, column=0, columnspan=2, pady=5)

    fn_getbox = Entry(fn_frame, font=(main_font, option_label_font), width=20, fg=text_color, bg=entry_bg)
    fn_getbox.grid(row=1, column=1, padx=5)
    fn_getbox_but = Button(fn_frame, font=(main_font, option_label_font, 'bold'), text="Search", width=10, borderwidth=10, fg=text_color, bg=button_bg, activebackground=button_act_bg, command=fn_search)
    fn_getbox_but.grid(row=1, column=0, padx=5)

    fn_edit = Entry(fn_frame, font=(main_font, option_label_font), width=20, fg=text_color, bg=entry_bg)
    fn_edit.grid(row=2, column=1, padx=5)
    fn_edit_label = Label(fn_frame, text="First Name", font=(main_font, option_label_font), width=10, fg=text_color, bg=frame_bg, padx=set_padx, pady=set_pady)
    fn_edit_label.grid(row=2, column=0)

    gender_edit = ttk.Combobox(fn_frame, font=(main_font, option_label_font), width=18, value=gend_comb, state="readonly")
    gender_edit.current(0)
    gender_edit.grid(row=3, column=1)
    gender_edit_label = Label(fn_frame, text="Gender", font=(main_font, option_label_font), width=10, fg=text_color, bg=frame_bg, padx=set_padx, pady=set_pady)
    gender_edit_label.grid(row=3, column=0)

    d1880s_edit = ttk.Combobox(fn_frame, font=(main_font, option_label_font), width=18, value=deccom_comb, state="readonly")
    d1880s_edit.current(0)
    d1880s_edit.grid(row=4, column=1)
    d1880s_edit_label = Label(fn_frame, text="1880s", font=(main_font, option_label_font), width=10, fg=text_color, bg=frame_bg, padx=set_padx, pady=set_pady)
    d1880s_edit_label.grid(row=4, column=0)

    d1890s_edit = ttk.Combobox(fn_frame, font=(main_font, option_label_font), width=18, value=deccom_comb, state="readonly")
    d1890s_edit.current(0)
    d1890s_edit.grid(row=5, column=1)
    d1890s_edit_label = Label(fn_frame, text="1890s", font=(main_font, option_label_font), width=10, fg=text_color, bg=frame_bg, padx=set_padx, pady=set_pady)
    d1890s_edit_label.grid(row=5, column=0)

    d1900s_edit = ttk.Combobox(fn_frame, font=(main_font, option_label_font), width=18, value=deccom_comb, state="readonly")
    d1900s_edit.current(0)
    d1900s_edit.grid(row=6, column=1)
    d1900s_edit_label = Label(fn_frame, text="1900s", font=(main_font, option_label_font), width=10, fg=text_color, bg=frame_bg, padx=set_padx, pady=set_pady)
    d1900s_edit_label.grid(row=6, column=0)

    d1910s_edit = ttk.Combobox(fn_frame, font=(main_font, option_label_font), width=18, value=deccom_comb, state="readonly")
    d1910s_edit.current(0)
    d1910s_edit.grid(row=7, column=1)
    d1910s_edit_label = Label(fn_frame, text="1910s", font=(main_font, option_label_font), width=10, fg=text_color, bg=frame_bg, padx=set_padx, pady=set_pady)
    d1910s_edit_label.grid(row=7, column=0)

    d1920s_edit = ttk.Combobox(fn_frame, font=(main_font, option_label_font), width=18, value=deccom_comb, state="readonly")
    d1920s_edit.current(0)
    d1920s_edit.grid(row=8, column=1)
    d1920s_edit_label = Label(fn_frame, text="1920s", font=(main_font, option_label_font), width=10, fg=text_color, bg=frame_bg, padx=set_padx, pady=set_pady)
    d1920s_edit_label.grid(row=8, column=0)

    d1930s_edit = ttk.Combobox(fn_frame, font=(main_font, option_label_font), width=18, value=deccom_comb, state="readonly")
    d1930s_edit.current(0)
    d1930s_edit.grid(row=9, column=1)
    d1930s_edit_label = Label(fn_frame, text="1930s", font=(main_font, option_label_font), width=10, fg=text_color, bg=frame_bg, padx=set_padx, pady=set_pady)
    d1930s_edit_label.grid(row=9, column=0)

    d1940s_edit = ttk.Combobox(fn_frame, font=(main_font, option_label_font), width=18, value=deccom_comb, state="readonly")
    d1940s_edit.current(0)
    d1940s_edit.grid(row=10, column=1)
    d1940s_edit_label = Label(fn_frame, text="1940s", font=(main_font, option_label_font), width=10, fg=text_color, bg=frame_bg, padx=set_padx, pady=set_pady)
    d1940s_edit_label.grid(row=10, column=0)

    d1950s_edit = ttk.Combobox(fn_frame, font=(main_font, option_label_font), width=18, value=deccom_comb, state="readonly")
    d1950s_edit.current(0)
    d1950s_edit.grid(row=11, column=1)
    d1950s_edit_label = Label(fn_frame, text="1950s", font=(main_font, option_label_font), width=10, fg=text_color, bg=frame_bg, padx=set_padx, pady=set_pady)
    d1950s_edit_label.grid(row=11, column=0)

    d1960s_edit = ttk.Combobox(fn_frame, font=(main_font, option_label_font), width=18, value=deccom_comb, state="readonly")
    d1960s_edit.current(0)
    d1960s_edit.grid(row=12, column=1)
    d1960s_edit_label = Label(fn_frame, text="1960s", font=(main_font, option_label_font), width=10, fg=text_color, bg=frame_bg, padx=set_padx, pady=set_pady)
    d1960s_edit_label.grid(row=12, column=0)

    d1970s_edit = ttk.Combobox(fn_frame, font=(main_font, option_label_font), width=18, value=deccom_comb, state="readonly")
    d1970s_edit.current(0)
    d1970s_edit.grid(row=13, column=1)
    d1970s_edit_label = Label(fn_frame, text="1970s", font=(main_font, option_label_font), width=10, fg=text_color, bg=frame_bg, padx=set_padx, pady=set_pady)
    d1970s_edit_label.grid(row=13, column=0)

    d1980s_edit = ttk.Combobox(fn_frame, font=(main_font, option_label_font), width=18, value=deccom_comb, state="readonly")
    d1980s_edit.current(0)
    d1980s_edit.grid(row=14, column=1)
    d1980s_edit_label = Label(fn_frame, text="1980s", font=(main_font, option_label_font), width=10, fg=text_color, bg=frame_bg, padx=set_padx, pady=set_pady)
    d1980s_edit_label.grid(row=14, column=0)

    d1990s_edit = ttk.Combobox(fn_frame, font=(main_font, option_label_font), width=18, value=deccom_comb, state="readonly")
    d1990s_edit.current(0)
    d1990s_edit.grid(row=15, column=1)
    d1990s_edit_label = Label(fn_frame, text="1990s", font=(main_font, option_label_font), width=10, fg=text_color, bg=frame_bg, padx=set_padx, pady=set_pady)
    d1990s_edit_label.grid(row=15, column=0)

    d2000s_edit = ttk.Combobox(fn_frame, font=(main_font, option_label_font), width=18, value=deccom_comb, state="readonly")
    d2000s_edit.current(0)
    d2000s_edit.grid(row=16, column=1)
    d2000s_edit_label = Label(fn_frame, text="2000s", font=(main_font, option_label_font), width=10, fg=text_color, bg=frame_bg, padx=set_padx, pady=set_pady)
    d2000s_edit_label.grid(row=16, column=0)

    d2010s_edit = ttk.Combobox(fn_frame, font=(main_font, option_label_font), width=18, value=deccom_comb, state="readonly")
    d2010s_edit.current(0)
    d2010s_edit.grid(row=17, column=1)
    d2010s_edit_label = Label(fn_frame, text="2010s", font=(main_font, option_label_font), width=10, fg=text_color, bg=frame_bg, padx=set_padx, pady=set_pady)
    d2010s_edit_label.grid(row=17, column=0)

    fn_submit_but = Button(fn_frame, font=(main_font, 13, 'bold'), text="Submit", width=10, pady=3, borderwidth=10, fg=text_color, bg=button_bg, activebackground=button_act_bg, command=fn_submit)
    fn_submit_but.grid(row=18, column=0, pady=5, columnspan=2)

    # Last Names

    ln_frame = LabelFrame(dbwin, font=(main_font, frame_label_font), borderwidth=5, padx=5, pady=5, fg=text_color, background=frame_bg, relief=RIDGE)
    ln_frame.grid(row=0, column=2, rowspan=2, columnspan=2, sticky=N)
    ln_label = Label(ln_frame, text="Last Names", font=(main_font, frame_label_font, 'bold'), fg=text_color, bg=frame_bg)
    ln_label.grid(row=0, column=0, columnspan=2, pady=5)

    ln_getbox = Entry(ln_frame, font=(main_font, option_label_font), width=20, fg=text_color, bg=entry_bg)
    ln_getbox.grid(row=1, column=1, padx=5)
    ln_getbox_but = Button(ln_frame, font=(main_font, option_label_font, 'bold'), text="Search", width=10, borderwidth=10, fg=text_color, bg=button_bg, activebackground=button_act_bg, command=ln_search)
    ln_getbox_but.grid(row=1, column=0, padx=5)

    ln_edit = Entry(ln_frame, font=(main_font, option_label_font), width=20, fg=text_color, bg=entry_bg)
    ln_edit.grid(row=2, column=1)
    ln_edit_label = Label(ln_frame, text="Last Name", font=(main_font, option_label_font), width=10, fg=text_color, bg=frame_bg, padx=set_padx, pady=set_pady)
    ln_edit_label.grid(row=2, column=0)

    ln_common_edit = ttk.Combobox(ln_frame, font=(main_font, option_label_font), width=18, value=lncom_comb, state="readonly")
    ln_common_edit.current(0)
    ln_common_edit.grid(row=3, column=1)
    ln_common_edit_label = Label(ln_frame, text="Commonness", font=(main_font, option_label_font), width=10, fg=text_color, bg=frame_bg, padx=set_padx, pady=set_pady)
    ln_common_edit_label.grid(row=3, column=0)

    origin_1_edit = Entry(ln_frame, font=(main_font, option_label_font), width=20, fg=text_color, bg=entry_bg)
    origin_1_edit.grid(row=4, column=1)
    origin_1_edit_label = Label(ln_frame, text="Origin 1", font=(main_font, option_label_font), width=10, fg=text_color, bg=frame_bg, padx=set_padx, pady=set_pady)
    origin_1_edit_label.grid(row=4, column=0)

    origin_2_edit = Entry(ln_frame, font=(main_font, option_label_font), width=20, fg=text_color, bg=entry_bg)
    origin_2_edit.grid(row=5, column=1)
    origin_2_edit_label = Label(ln_frame, text="Origin 2", font=(main_font, option_label_font), width=10, fg=text_color, bg=frame_bg, padx=set_padx, pady=set_pady)
    origin_2_edit_label.grid(row=5, column=0)

    origin_3_edit = Entry(ln_frame, font=(main_font, option_label_font), width=20, fg=text_color, bg=entry_bg)
    origin_3_edit.grid(row=6, column=1)
    origin_3_edit_label = Label(ln_frame, text="Origin 3", font=(main_font, option_label_font), width=10, fg=text_color, bg=frame_bg, padx=set_padx, pady=set_pady)
    origin_3_edit_label.grid(row=6, column=0)

    ln_submit_but = Button(ln_frame, font=(main_font, 13, 'bold'), text="Submit", width=10, pady=3, borderwidth=10, fg=text_color, bg=button_bg, activebackground=button_act_bg, command=ln_submit)
    ln_submit_but.grid(row=20, column=0, pady=5, columnspan=2)

    # Adjectives

    adj_frame = LabelFrame(dbwin, font=(main_font, frame_label_font), borderwidth=5, padx=5, pady=5, fg=text_color, background=frame_bg, relief=RIDGE)
    adj_frame.grid(row=2, column=2, columnspan=2, sticky=N)
    adj_label = Label(adj_frame, text="Adjectives", font=(main_font, frame_label_font, 'bold'), fg=text_color, bg=frame_bg)
    adj_label.grid(row=0, column=0, columnspan=2, pady=5)

    adj_getbox = Entry(adj_frame, font=(main_font, option_label_font), width=20, fg=text_color, bg=entry_bg)
    adj_getbox.grid(row=1, column=1, padx=5)
    adj_getbox_but = Button(adj_frame, font=(main_font, option_label_font, 'bold'), text="Search", width=10, borderwidth=10, fg=text_color, bg=button_bg, activebackground=button_act_bg, command=adj_search)
    adj_getbox_but.grid(row=1, column=0, padx=5)

    adj_edit = Entry(adj_frame, font=(main_font, option_label_font), width=20, fg=text_color, bg=entry_bg)
    adj_edit.grid(row=2, column=1)
    adj_edit_label = Label(adj_frame, text="Adjective", font=(main_font, option_label_font), width=10, fg=text_color, bg=frame_bg,  padx=set_padx, pady=set_pady)
    adj_edit_label.grid(row=2, column=0)

    adj_submit_but = Button(adj_frame, font=(main_font, 13, 'bold'), text="Submit", width=10, pady=3, borderwidth=10, fg=text_color, bg=button_bg, activebackground=button_act_bg, command=adj_submit)
    adj_submit_but.grid(row=18, column=0, pady=5, columnspan=2)

    # Nouns

    nns_frame = LabelFrame(dbwin, font=(main_font, frame_label_font), borderwidth=5, padx=5, pady=5, fg=text_color, background=frame_bg, relief=RIDGE)
    nns_frame.grid(row=3, column=2, columnspan=2, sticky=N)
    nns_label = Label(nns_frame, text="Nouns", font=(main_font, frame_label_font, 'bold'), fg=text_color, bg=frame_bg)
    nns_label.grid(row=0, column=0, columnspan=2, pady=5)

    nns_getbox = Entry(nns_frame, font=(main_font, option_label_font), width=20, fg=text_color, bg=entry_bg)
    nns_getbox.grid(row=1, column=1, padx=5)
    nns_getbox_but = Button(nns_frame, font=(main_font, option_label_font, 'bold'), text="Search", width=10, borderwidth=10, fg=text_color, bg=button_bg, activebackground=button_act_bg, command=nns_search)
    nns_getbox_but.grid(row=1, column=0, padx=5)

    nns_edit = Entry(nns_frame, font=(main_font, option_label_font), width=20, fg=text_color, bg=entry_bg)
    nns_edit.grid(row=2, column=1)
    nns_edit_label = Label(nns_frame, text="Noun", font=(main_font, option_label_font), width=10, fg=text_color, bg=frame_bg, padx=set_padx, pady=set_pady)
    nns_edit_label.grid(row=2, column=0)

    ntype_edit = ttk.Combobox(nns_frame, font=(main_font, option_label_font), width=18, value=ntype_comb, state="readonly")
    ntype_edit.current(0)
    ntype_edit.grid(row=3, column=1)
    ntype_edit_label = Label(nns_frame, text="Type", font=(main_font, option_label_font), width=10, fg=text_color, bg=frame_bg, padx=set_padx, pady=set_pady)
    ntype_edit_label.grid(row=3, column=0)

    nns_submit_but = Button(nns_frame, font=(main_font, 13, 'bold'), text="Submit", width=10, pady=3, borderwidth=10, fg=text_color, bg=button_bg, activebackground=button_act_bg, command=nns_submit)
    nns_submit_but.grid(row=18, column=0, pady=5, columnspan=2)

    # Main Buttons

    clear_but = Button(dbwin, font=(main_font, 18, 'bold'), text="Clear", width=8, borderwidth=20, padx=5, pady=10, fg=text_color, background=frame_bg, relief=RAISED, command=clear_all)
    clear_but.grid(row=3, column=0, sticky=S)
    close_but = Button(dbwin, font=(main_font, 18, 'bold'), text="Close", width=8, borderwidth=20, padx=5, pady=10, fg=text_color, background=frame_bg, relief=RAISED, command=dbwin.destroy)
    close_but.grid(row=3, column=1, sticky=S)

    dbwin.mainloop()


db_button = Button(root, font=(main_font, 18, 'bold'), text="Database\nManagement", fg=text_color, bg=button_bg, activebackground=button_act_bg, relief=RAISED, borderwidth=18, command=database)
...
file_menu.add_command(label="About GeNamenator...", command=about_window)
file_menu.add_separator()
file_menu.add_command(label="Exit GeNemantor", command=root.quit)
