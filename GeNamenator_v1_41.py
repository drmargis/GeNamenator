...
print("GeNamenator, version 1.41")
...
def description():
    ...
    adject = cursor.fetchall()
    cursor.execute(" SELECT NOUN FROM nouns ")
    noun = cursor.fetchall()
    print("the " + random.choice(adject) + " " + random.choice(noun))


def alit_description(end_string):
    cursor.execute(" SELECT ADJECTIVE from ADJECTIVES WHERE ADJECTIVE LIKE (?)", (end_string,))
    adject = cursor.fetchall()
    cursor.execute(" SELECT NOUN FROM nouns WHERE NOUN LIKE (?)", (end_string,))
    noun = cursor.fetchall()
    print("the " + random.choice(adject) + " " + random.choice(noun))
